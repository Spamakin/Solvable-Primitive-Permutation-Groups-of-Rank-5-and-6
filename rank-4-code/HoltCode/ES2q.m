...
MaxQPrimSolSubsRQ := function(G,phi,K) 
  ...
  GG := G @@ phi;
  V := GModule(GG);
  if (#GG le 10^7 or IsSolvable(G)) and not NoRegOrb(G @@ phi,V) then
    return [];
  end if;
  gps := []; cands := [G];
...
"Line 113, b=4, p=3, a=1, d=8";
for q in [3] do
  GG := ESN(q);
  F := FittingSubgroup(GG);
  GG := TensorProduct(GG,GL(4,q));
  FG :=  TensorProduct(F,sub<GL(4,q)|>);
  K := FG;
  Q, phi := quo< GG | K >;
  S := MaxQPrimSolSubsRQ(Q, phi, K); #S;
  V := VectorSpace(GG);
